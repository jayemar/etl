...
class Loader(Model):
    ...
    def retrieve_data(ml_cfg):
        if type(ml_cfg) == str:
            with open(ml_cfg, 'r') as f:
                self.ml_cfg = yaml.load(f)

    def _get_data_gen(self, data_type):
        """Return a data generator for data type of either 'train' or 'test'"""
        train_gen = load_data(data_type, cfg.get('num_records'))

    def _load_data(self, data_type, num_lines):
        train_gen = get_data_generator('train', cfg.get('num_records'))
        train, meta_train = next(train_gen)
        test_gen = get_data_generator('test', cfg.get('num_records'))
